...
def generate_random_expander(
    data, degree, algorithm, rng=None, max_num_iters=100, exp_index=0
):
    ...
    if num_nodes <= 10:
        ...
        while eig_val < eig_val_lower_bound and cur_iter <= max_num_iters:
            if algorithm == "Random-d":
                senders, receivers = generate_random_regular_graph1(
                    num_nodes, degree, rng
                )
            elif algorithm == "Random-d-2":
                senders, receivers = generate_random_regular_graph2(
                    num_nodes, degree, rng
                )
                ...
                raise ValueError(
                    "prep.exp_algorithm should be one of the Random-d or Hamiltonian"
                )
