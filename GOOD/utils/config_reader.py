    ...
    for key in config.keys():
        if type(config[key]) is dict:
            ...
            if found:
                ...
                warnings.warn(
                    f"Argument {key} in the chosen config yaml file are not defined in command arguments, "
...
def process_configs(config: Union[CommonArgs, Munch]):
    ...
    config.tensorboard_logdir = opj(
        STORAGE_DIR, "tensorboard", f"{config.dataset.dataset_name}"
    )
    ...
    dataset_dirname = config.dataset.dataset_name + "_" + str(config.dataset.domain)
    if config.dataset.shift_type:
    ...
    log_dirs = opj(
        log_dir_root, dataset_dirname, model_dirname, train_dirname, ood_dirname
    )
    if config.save_tag:
    ...
    if config.ckpt_dir is None:
        ...
        config.ckpt_dir = opj(
            config.ckpt_dir, dataset_dirname, model_dirname, train_dirname, ood_dirname
        )
        if config.save_tag:
    ...
    config.device = torch.device(
        f"cuda:{config.gpu_idx}" if torch.cuda.is_available() else "cpu"
    )
