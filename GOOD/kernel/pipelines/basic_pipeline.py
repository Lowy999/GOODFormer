...
class Pipeline:
    ...
    def train_batch(self, data: Batch, pbar) -> dict:
        ...
        node_norm = (
            data.get("node_norm") if self.config.model.model_level == "node" else None
        )
        ...
        edge_weight = (
            data.get("edge_norm") if self.config.model.model_level == "node" else None
        )

        model_output = self.model(
            data=data, edge_weight=edge_weight, ood_algorithm=self.ood_algorithm
        )
        raw_pred = self.ood_algorithm.output_postprocess(model_output)

        loss = self.ood_algorithm.loss_calculate(
            raw_pred, targets, mask, node_norm, self.config
        )
    ...
    def train(self):
        ...
        for epoch in range(self.config.train.ctn_epoch, self.config.train.max_epoch):
            ...
            pbar = enumerate(self.loader["train"])
            for index, data in pbar:
                ...
                # Parameter for DANN
                p = (
                    index / len(self.loader["train"]) + epoch
                ) / self.config.train.max_epoch
                self.config.train.alpha = 2.0 / (1.0 + np.exp(-10 * p)) - 1

                self.ood_algorithm.preprocess(self.loader["train"])
            ...
            id_val_stat = {"score": None, "loss": None}
            id_test_stat = {"score": None, "loss": None}
    ...
    def evaluate(self, split: str):
        ...
        for data in pbar:
            ...
            if mask is None:
                return stat
            node_norm = (
                torch.ones((data.num_nodes,), device=self.config.device)
            ...
            if self.config.ood.ood_alg == "GOODFormerAlg":
                with torch.set_grad_enabled(False):
                    copy_data = self.model.get_node_rep(copy.deepcopy(data))
                    _, attn_mask, causal_attention_logits, conf_attention_logits = (
                        self.model.disentanglement_transformer_layers[
                            0
                        ].get_attention_logits(copy_data)
                    )
                temperature = self.optimize_temperature(
                    data, attn_mask, causal_attention_logits, conf_attention_logits
                )
                with torch.set_grad_enabled(False):
                    model_output = self.model(
                        data=data,
            ...
            mask_all.append(mask[:current_bs])
            loss_all.append(loss[:current_bs])
            # ------------- Score data collection ------------------
            pred, target = eval_data_preprocess(data.y, raw_preds, mask, self.config)
            pred_all.append(pred[:current_bs])
            target_all.append(target[:current_bs])
        # ------- Loss calculate -------
    ...
    def optimize_temperature(
        self, data, attn_mask, causal_attention_logits, conf_attention_logits
    ):
        ...
        for _ in range(10):
            (
                ...
                _,
                conf_attn_entropy,
            ) = self.model.disentanglement_transformer_layers[0].softmax(
                attn_mask, causal_attention_logits, conf_attention_logits, temperature
            )
            loss = torch.norm(
                self.model.mean_causal_attention_entropy - causal_attn_entropy,
                p=1,
            ) + torch.norm(
                self.model.mean_conf_attention_entropy - conf_attn_entropy,
                p=1,
            )
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        return torch.clamp(temperature, min=1e-9)
    ...
    def load_task(self):
        ...
        if self.task == "train":
            ...
            test_score, test_loss = self.config_model(self.task)
    ...
    def config_model(self, mode: str, load_param=False):
        ...
        if mode == "test":
            ...
            if os.path.exists(self.config.id_test_ckpt):
                ...
                print(
                    f'#IN#ChartInfo {id_ckpt["id_test_score"]:.4f} {id_ckpt["test_score"]:.4f} '
                ...
                print(
                    f'#IN#ChartInfo {ckpt["test_score"]:.4f} {ckpt["val_score"]:.4f}',
                    end="",
                )
            if load_param:
                self.model.load_state_dict(ckpt["state_dict"])
    ...
    def save_epoch(
        ...
        if not (
            ...
            or config.metric.lower_better * val_stat["score"]
            < config.metric.lower_better * config.metric.best_stat["score"]
            or (
                id_val_stat.get("score")
                and (
                    config.metric.id_best_stat["score"] is None
                    or config.metric.lower_better * id_val_stat["score"]
                    < config.metric.lower_better * config.metric.id_best_stat["score"]
                )
            )
